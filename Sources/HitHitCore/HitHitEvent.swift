...
/// 수집 이벤트의 종류.
public enum HitHitEventType: String, Codable, Sendable {
    case tap
    case scroll
}

/// 디바이스 방향. 같은 방향끼리만 히트맵 집계가 의미를 가진다.
public enum HitHitOrientation: String, Codable, Sendable {
    case portrait
    case landscape
}
...
public struct HitHitEvent: Codable, Equatable, Sendable {
    ...
    public let type: HitHitEventType
    ...
    public init(
        schemaVersion: Int = HitHitEvent.currentSchemaVersion,
        id: String = UUID().uuidString,
        type: HitHitEventType,
        screen: String,
        x: Double?,
        y: Double?,
        scrollDepth: Double?,
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.schemaVersion = try c.decodeIfPresent(Int.self, forKey: .schemaVersion) ?? HitHitEvent.currentSchemaVersion
        self.id = try c.decodeIfPresent(String.self, forKey: .id) ?? UUID().uuidString
        self.type = try c.decode(HitHitEventType.self, forKey: .type)
        ...
        self.orientation = try c.decode(HitHitOrientation.self, forKey: .orientation)
        self.ts = try c.decode(Int64.self, forKey: .ts)
    }
}

public extension HitHitEvent {
