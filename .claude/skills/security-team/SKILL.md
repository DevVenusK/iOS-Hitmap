...
- 이벤트를 로컬 JSONL로 배치 저장하고, 호스트가 제공하는 `HitHitUploader`에 배치를 넘깁니다.
- 옵셔널 `HitHitViz` 타겟이 화면 배경 스크린샷 위에 히트맵을 렌더링합니다.
...
6. **업로더 경계**: 기기를 벗어나는 데이터의 정확한 범위와 책임 분리(`HitHitUploader` 프로토콜).
...
- **명확한 입장**: 이 SDK는 **ATT/IDFA 기반 크로스앱 추적이 아니다** (1st-party 분석 전용, `docs/sdk-spec/hithitkit.md` §2 Non-Goals 명시). 다만 추적이 아니라는 이유로 매니페스트 제출을 생략하지 않습니다 — 매니페스트는 여전히 필수.

**입력 문서**:
- `docs/sdk-spec/hithitkit.md` — 특히 **§11 보안/안전성**, **§4.2 ElementIDPolicy / HitHitConfig**, **§4.4 에러 모델**, §12 미결사항(elementID PII 분류 법무 검토 미결 명시됨)
...
- **강점**: iOS secure coding, ElementIDPolicy/HitHitConfig 같은 SDK API 레벨 보안 설계, PrivacyInfo.xcprivacy 작성, CI 보안 게이트(secret scan/SCA), 로컬 저장(Keychain/캐시 디렉터리) 보안
...
| 6 | 업로더 경계 | `HitHitUploader`로 넘어가는 배치에 정책 적용 후에도 남는 잔존 정보가 있는가? |
...
- **보안 아키텍처 계층** (이 SDK에 맞게 재해석):
  ...
  - Upload Boundary (HitHitUploader 경계 — 무엇이, 어떤 형태로 기기를 벗어나는가)
  - Compliance Artifact (PrivacyInfo.xcprivacy 매니페스트)
  - Error Handling (HitHitError — 민감정보 누출 없는 에러 메시지)
...
### 📋 입력 컨텍스트 확인
- 보호 대상: HitHitKit SDK (Finda 등 금융 호스트앱에 내장)
...
| T-5 | HitHitUploader 구현체가 TLS 미적용/평문 전송 | Information disclosure + Tampering | 업로드 배치 | MITM 공격자 | M (SDK 책임 경계 밖이나 가이드 필요) | 업로더 가이드라인에 TLS 필수 명시, 샘플 업로더에 강제 |
| T-6 | HitHitError가 내부 경로/스택을 노출 | Information disclosure | 에러 메시지 | 로그 열람자 | L | struct+code 모델 유지, message는 generic |
| ... | | | | | | |
...
| Upload Boundary | HitHitUploader 가이드라인 + 샘플 구현 TLS 강제 | T-5 | 낮음(문서) | 없음 |
| Error Handling | HitHitError struct+code, generic message | T-6 | 0 | 없음 |
...
| 위험 | 수용 사유 | 검토 주기 |
|---|---|---|
| HitHitUploader 구현 자체의 전송 보안은 SDK 책임 경계 밖 | 호스트 책임 분리 설계(인터페이스만 제공), 가이드라인+샘플로 완화 | 마이너 릴리즈마다 |
...
```markdown
# Security Spec: HitHitKit <기능명 또는 1.0 전체>

## 1. 개요
- 보호 대상: HitHitKit SDK (호스트: <Finda 등>)
...
### Non-Goals
- (예: 호스트 앱의 네트워크 계층 보안은 본 스펙 범위 밖 — HitHitUploader 가이드라인으로 위임)
...
- `excludedScreenIDs`가 수집과 `HitHitRenderRequest.background` 양쪽에 동일하게 적용되는 구조:
...
### 6.6 Upload Boundary (HitHitUploader)
- `HitHitUploader` 프로토콜 경계에서 SDK가 보증하는 것 / 호스트 책임으로 위임하는 것:
- 샘플 업로더의 TLS 강제 여부:

### 6.7 Error Handling
- `HitHitError` 코드별 메시지에 내부 경로/PII가 섞이지 않는지:
...
**🔴 Senior (한시니어)**
"CTO님, HitHitKit 보안 스펙 검토 요청드립니다.
- 보호 대상: HitHitKit SDK — 금융 호스트앱 내 전역 터치 인터셉트
...
**구현 산출물 분류:**
- **코드 (Code)**: Consent Gate, ElementIDPolicy 적용 로직, 민감화면 필터, HitHitError 처리
...
|---|---|---|---|
| C-1 | Consent Gate (수집 최상단 강제) | Sources/HitHitKit/HitHitTracker.swift:NN | Code + Test |
| C-2 | ElementIDPolicy 기본 hashed | Sources/HitHitKit/ElementIDPolicy.swift:NN | Code + Test |
...
| Secret/PII 노출 (코드/로그/HitHitError) | ✅/⚠️/❌ | |
| 암호화/해싱 알고리즘 최신성 | ✅/⚠️/❌ | |
...
- `HitHitError.message`에 elementID 원문, 좌표, 내부 경로 등을 포함하지 않습니다. 식별은 `code`로.
...
- "정상 수집 동작" 만 테스트하지 말고:
  ...
  - 업로더 경계 → 정책 적용 후 데이터만 `HitHitUploader`에 전달됨
